...
function selectStyleElement() {
  ...
  if (styleElement === "gridOverlay") {
    ...
    if (styleGridShowNumbers) styleGridShowNumbers.checked = el.attr("data-show-numbers") === "1";
    if (styleGridNumberSize) styleGridNumberSize.value = el.attr("data-number-size") || 8;
    if (styleGridNumberColor) {
...
// Grid Numbering Event Handlers
// These need to be re-added after variables are defined or safely queried
(function addGridNumberingHandlers() {
  const styleGridShowNumbers = byId("styleGridShowNumbers");
  const styleGridNumberSize = byId("styleGridNumberSize");
  const styleGridNumberColor = byId("styleGridNumberColor");
  const styleGridNumberColorOutput = byId("styleGridNumberColorOutput");

  if (styleGridShowNumbers) {
